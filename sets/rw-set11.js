/* SAT Studio question set — Reading & Writing: Rhetorical Synthesis (RHM-01 a RHM-10) */
window.SAT_SETS.push({
  id: 'rw-set11',
  title: 'Rhetorical Synthesis — Medium',
  section: 'rw',
  level: 'Media',
  description: 'Expression of Ideas: synthesize notes to meet a goal — intermediate level.',
  minutes: 13,
  questions: [
    {
      id:'RHM-01', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>The zoetrope is a spinning cylinder with slits cut into its sides.</li><li>A strip of drawings lines the inside wall of the cylinder.</li><li>Viewers look through the moving slits as the cylinder spins.</li><li>The rapid glimpses make the still drawings appear to move.</li></ul>The student wants to explain how a zoetrope creates the illusion of motion. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'As the cylinder spins, viewers look through the moving slits, and the rapid glimpses make the still drawings appear to move.',
        B:'The zoetrope is a spinning cylinder with slits cut into its sides.',
        C:'A strip of drawings lines the inside wall of the zoetrope’s cylinder.',
        D:'The zoetrope, an early animation device, delighted audiences long before film.'
      },
      correct:'A',
      expCorrect:'La meta pide explicar CÓMO se crea la ilusión de movimiento. La A describe el mecanismo completo: al girar el cilindro, el espectador mira por las ranuras en movimiento y esos vistazos rápidos hacen que los dibujos quietos parezcan moverse. Cumple la meta con notas exactas.',
      expWrong:{
        B:'Describe la forma del aparato, pero no explica cómo produce el movimiento.',
        C:'Da un detalle de dónde van los dibujos; no explica el efecto de movimiento.',
        D:'Es una afirmación general sin datos de las notas; no explica el mecanismo.'
      },
      tip:'"Explicar cómo" = busca la opción con el MECANISMO paso a paso, no la que solo describe el objeto.'
    },
    {
      id:'RHM-02', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>Podcasts are digital audio programs released in episodes.</li><li>Listeners can \"subscribe\" so new episodes download automatically.</li><li>Radio broadcasts air at fixed times set by the station.</li><li>Podcast listeners can play any episode whenever they choose.</li><li>Radio listeners must tune in when a show is broadcast.</li></ul>The student wants to compare how podcasts and radio differ in when listeners can hear them. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'Podcasts are digital audio programs released in episodes that listeners can subscribe to.',
        B:'Whereas radio listeners must tune in when a show is broadcast, podcast listeners can play any episode whenever they choose.',
        C:'Radio broadcasts air at fixed times set by the station.',
        D:'When listeners subscribe to a podcast, new episodes download automatically.'
      },
      correct:'B',
      expCorrect:'La meta pide COMPARAR cuándo se puede escuchar cada uno. La B pone los dos lados frente a frente: la radio obliga a sintonizar a la hora del programa, mientras que el podcast se escucha cuando uno quiera. Compara ambos con notas exactas.',
      expWrong:{
        A:'Solo describe el podcast; falta el lado de la radio para comparar.',
        C:'Solo habla de la radio; no menciona el podcast, así que no compara.',
        D:'Da un detalle de la suscripción; no compara los dos medios en cuanto al momento de escuchar.'
      },
      tip:'"Comparar dos cosas" = la opción correcta debe mencionar AMBAS, no solo una.'
    },
    {
      id:'RHM-03', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>A researcher tracked two groups of teens for one month.</li><li>Group 1 read printed books; Group 2 read the same texts on phones.</li><li>Afterward, both groups took a comprehension quiz.</li><li>Group 1 (print) scored an average of 82%.</li><li>Group 2 (phones) scored an average of 71%.</li></ul>The student wants to present a specific finding from the study. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'A researcher tracked two groups of teens over the course of one month.',
        B:'One group read printed books while the other read the same texts on phones.',
        C:'Readers who used printed books scored 82% on the comprehension quiz, compared with 71% for those who read on phones.',
        D:'The medium a person reads on can affect how well they understand a text.'
      },
      correct:'C',
      expCorrect:'La meta pide un HALLAZGO específico. La C da los números concretos del estudio: 82% para quienes leyeron en papel frente a 71% para quienes leyeron en el teléfono. Es un resultado medible y exacto.',
      expWrong:{
        A:'Describe el montaje del estudio, no lo que se descubrió.',
        B:'Explica cómo se dividieron los grupos, pero no reporta ningún resultado con datos.',
        D:'Es una conclusión general y vaga; no presenta el hallazgo específico con cifras.'
      },
      tip:'"Hallazgo específico" = busca la opción con los DATOS/números concretos, no la que describe el diseño.'
    },
    {
      id:'RHM-04', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>Emoji are small digital images used in text messages.</li><li>The first emoji set was created in Japan in 1999 by Shigetaka Kurita.</li><li>It contained 176 simple icons.</li><li>Emoji are now standardized by a nonprofit called the Unicode Consortium.</li><li>Standardization lets an emoji appear correctly across different phone brands.</li></ul>The student wants to explain why emoji look the same across different phone brands. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'The first emoji set, created in Japan in 1999, contained 176 simple icons.',
        B:'Emoji are small digital images used in text messages around the world.',
        C:'Because the Unicode Consortium standardizes emoji, a given emoji appears correctly across different phone brands.',
        D:'Emoji were invented by Shigetaka Kurita, a Japanese designer.'
      },
      correct:'C',
      expCorrect:'La meta pide explicar POR QUÉ los emoji se ven igual en distintas marcas. La C da la causa: como el Unicode Consortium los estandariza, un emoji aparece correctamente en teléfonos de marcas diferentes. Cumple la meta con notas exactas.',
      expWrong:{
        A:'Da la historia y el número original de iconos; no explica la consistencia entre marcas.',
        B:'Define qué son los emoji, pero no dice por qué se ven igual en todos lados.',
        D:'Nombra al creador; es un dato histórico, no la causa de la estandarización.'
      },
      tip:'"Explicar por qué" = busca la opción que da la CAUSA (aquí: la estandarización), no la historia ni la definición.'
    },
    {
      id:'RHM-05', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>Stop-motion animation photographs physical models one frame at a time.</li><li>Between shots, animators move the models slightly by hand.</li><li>Computer animation builds and moves characters entirely inside software.</li><li>A single stop-motion film can require moving models tens of thousands of times.</li><li>Computer animation can generate many frames automatically.</li></ul>The student wants to contrast the effort required by stop-motion and computer animation. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'While stop-motion animators must move models by hand tens of thousands of times, computer animation can generate many frames automatically.',
        B:'Stop-motion animation photographs physical models one frame at a time.',
        C:'Computer animation builds and moves characters entirely inside software.',
        D:'Both stop-motion and computer animation are used to make animated films.'
      },
      correct:'A',
      expCorrect:'La meta pide CONTRASTAR el esfuerzo de cada técnica. La A usa "While" para oponer los dos lados: el stop-motion exige mover modelos a mano decenas de miles de veces, mientras que la computadora genera muchos cuadros automáticamente. Cumple la meta con notas exactas.',
      expWrong:{
        B:'Solo describe cómo funciona el stop-motion; falta el contraste con la animación por computadora.',
        C:'Solo describe la animación por computadora; falta el otro lado del contraste.',
        D:'Junta ambas técnicas, pero solo dice que comparten un uso; no contrasta el esfuerzo.'
      },
      tip:'"Contrastar" = busca la palabra de oposición (While, Whereas, Although) que junte los DOS lados.'
    },
    {
      id:'RHM-06', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>Braille is a writing system read by touch.</li><li>It uses raised dots arranged in cells of up to six dots.</li><li>Louis Braille invented it in 1824, when he was fifteen years old.</li><li>Braille had lost his sight in a childhood accident.</li><li>The system is now used for dozens of languages worldwide.</li></ul>The student wants to introduce Braille to an audience unfamiliar with it. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'Louis Braille had lost his sight in a childhood accident before inventing the system.',
        B:'Braille is a writing system read by touch that uses raised dots arranged in cells of up to six dots.',
        C:'Louis Braille invented the system in 1824, when he was only fifteen years old.',
        D:'Braille is now used for dozens of languages around the world.'
      },
      correct:'B',
      expCorrect:'La meta es PRESENTAR el Braille a alguien que no lo conoce. La B da lo esencial: qué es (un sistema de escritura que se lee con el tacto) y cómo funciona (puntos en relieve en celdas de hasta seis). Es la mejor introducción con notas exactas.',
      expWrong:{
        A:'Habla de la vida del inventor; es un detalle biográfico, no una presentación del sistema.',
        C:'Da la fecha y la edad del inventor; es un dato histórico, no una introducción general.',
        D:'Menciona el alcance actual, pero no explica qué es ni cómo funciona el Braille.'
      },
      tip:'"Introducir a un público que no conoce" = elige lo más básico (qué es y cómo funciona), no un detalle histórico.'
    },
    {
      id:'RHM-07', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>Vinyl record sales had nearly disappeared by the early 2000s.</li><li>In 2005, about 900,000 vinyl records were sold in the United States.</li><li>Sales then rose steadily for years.</li><li>By 2022, more than 41 million vinyl records were sold.</li><li>Many buyers say they enjoy the physical ritual of playing a record.</li></ul>The student wants to emphasize how dramatically vinyl record sales grew. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'Vinyl record sales had nearly disappeared by the early 2000s.',
        B:'Many buyers say they enjoy the physical ritual of playing a record.',
        C:'Vinyl sales climbed from about 900,000 records in 2005 to more than 41 million in 2022.',
        D:'After years of decline, vinyl records became popular again.'
      },
      correct:'C',
      expCorrect:'La meta es enfatizar cuán DRAMÁTICO fue el crecimiento. La C junta las dos cifras clave: de unos 900,000 discos en 2005 a más de 41 millones en 2022. El salto en números subraya el crecimiento. Cumple la meta con notas exactas.',
      expWrong:{
        A:'Describe el punto bajo, pero no muestra el crecimiento posterior.',
        B:'Explica por qué la gente compra vinilos; no muestra el aumento de ventas.',
        D:'Dice que volvió a ser popular, pero es vago; no cuantifica el crecimiento.'
      },
      tip:'"Enfatizar cuánto creció" = escoge la opción con los DOS números (antes vs. después), no una afirmación vaga.'
    },
    {
      id:'RHM-08', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>Movie subtitles display translated dialogue as text on screen.</li><li>Dubbing replaces the original voices with recorded voices in another language.</li><li>Subtitles let viewers hear the actors’ original voices.</li><li>Dubbing lets viewers watch without reading text.</li><li>Some viewers find subtitles distracting; others dislike mismatched lip movements in dubbing.</li></ul>The student wants to compare subtitles and dubbing as ways to translate a film. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'Movie subtitles display translated dialogue as text on the screen.',
        B:'Some viewers find subtitles distracting to read during a film.',
        C:'Dubbing replaces the original voices with recorded voices in another language.',
        D:'Subtitles let viewers hear the actors’ original voices, whereas dubbing lets viewers watch without reading text.'
      },
      correct:'D',
      expCorrect:'La meta pide COMPARAR subtítulos y doblaje. La D pone ambos lados frente a frente: los subtítulos dejan oír las voces originales, mientras que el doblaje deja ver sin leer. Compara los dos métodos con notas exactas.',
      expWrong:{
        A:'Solo define qué son los subtítulos; falta el doblaje para comparar.',
        B:'Da una queja sobre los subtítulos; no compara con el doblaje.',
        C:'Solo describe el doblaje; no menciona los subtítulos, así que no compara.'
      },
      tip:'"Comparar dos cosas" = la opción correcta debe mencionar AMBAS, no solo una.'
    },
    {
      id:'RHM-09', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>QR codes are square patterns of black and white squares.</li><li>A phone camera scans the pattern to read stored data.</li><li>The data usually links to a website or menu.</li><li>QR codes include built-in error correction.</li><li>Error correction lets a code still be read even if part of it is dirty or damaged.</li></ul>The student wants to explain why a QR code can still work when part of it is damaged. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'QR codes are square patterns of black and white squares scanned by a phone camera.',
        B:'The data in a QR code usually links to a website or a menu.',
        C:'A phone camera scans the QR pattern to read the data stored inside it.',
        D:'Thanks to built-in error correction, a QR code can still be read even when part of it is dirty or damaged.'
      },
      correct:'D',
      expCorrect:'La meta pide explicar POR QUÉ un código QR sigue funcionando dañado. La D da la causa: gracias a la corrección de errores incorporada, el código se lee aunque una parte esté sucia o dañada. Cumple la meta con notas exactas.',
      expWrong:{
        A:'Describe cómo se ve y se escanea el código; no explica por qué resiste el daño.',
        B:'Dice a qué enlaza el código; no explica su resistencia al daño.',
        C:'Describe el escaneo con la cámara; no explica la corrección de errores.'
      },
      tip:'"Explicar por qué" = busca la nota que da la RAZÓN (aquí: la corrección de errores), no la que describe el aspecto o el uso.'
    },
    {
      id:'RHM-10', type:'mc', domain:'Expression of Ideas', difficulty:'Media',
      skill:'Rhetorical Synthesis',
      passage:"",
      stem:"While researching a topic, a student has taken the following notes:<ul><li>The Rosetta Stone is a carved slab found in Egypt in 1799.</li><li>It records the same text in three scripts: hieroglyphic, Demotic, and Greek.</li><li>Scholars could already read ancient Greek.</li><li>By matching the Greek to the other two scripts, they decoded Egyptian hieroglyphs.</li><li>The stone is now displayed at the British Museum.</li></ul>The student wants to explain how the Rosetta Stone helped scholars decode hieroglyphs. Which choice most effectively uses relevant information from the notes to accomplish this goal?",
      choices:{
        A:'The Rosetta Stone is a carved slab that was found in Egypt in 1799.',
        B:'Because scholars could already read the Greek text, they matched it to the other two scripts and decoded Egyptian hieroglyphs.',
        C:'The Rosetta Stone records the same text in hieroglyphic, Demotic, and Greek scripts.',
        D:'The Rosetta Stone is now displayed at the British Museum in London.'
      },
      correct:'B',
      expCorrect:'La meta pide explicar CÓMO la piedra ayudó a descifrar los jeroglíficos. La B da el método: como los eruditos ya sabían leer griego, lo compararon con las otras dos escrituras y así descifraron los jeroglíficos. Cumple la meta con notas exactas.',
      expWrong:{
        A:'Da cuándo y dónde se encontró; es un dato histórico, no el método de descifrado.',
        C:'Dice que tiene el mismo texto en tres escrituras; es cierto por las notas, pero no explica el proceso de descifrado (persigue otra meta).',
        D:'Indica dónde está hoy; no explica cómo se descifraron los jeroglíficos.'
      },
      tip:'"Explicar cómo" = busca la opción que describe el PROCESO paso a paso (usar el griego para descifrar el resto), no un dato suelto.'
    }
  ]
});
